...
type ModalCreateVersionProps = {
  onClose: () => void;
  onVersionCreated: () => void; // Giữ nguyên prop này
};
...
const ModalCreateVersion: React.FC<ModalCreateVersionProps> = ({ onClose, onVersionCreated }) => {
  const {
    register,
    handleSubmit,
    control,
    ...
    <div className="fixed inset-0 flex items-center justify-center">
      ...
      <div className="bg-white fixed border rounded w-[600px]">
        <div className="flex justify-between items-center m-1 bg-primary-lightBlue">
          ...
          <button className="icon bg-white w-5 h-5 rounded-sm me-1" onClick={onClose}></button>
        </div>
        <form onSubmit={handleSubmit(onSubmit)} className="detail p-4">
          {errors.name && (
            <p className="text-red-500 text-sm w-full border-primary-borderError bg-primary-bgError border-2 ps-4 mb-4">{errors.name.message}</p>
          )}
