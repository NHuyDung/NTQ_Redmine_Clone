...
const buttoncss = "bg-white p-2 border-2 rounded text-[#628db6] font-bold text-xs hover:border-primary-lightBlue";
...
const renderSection = (title: string, content: JSX.Element | string, isLink: boolean, link: string, onClick?: () => void) => (
  <>
...
const renderLink = (to: string, text: string, index: number) => (
  <Link key={index} to={to} className="text-header font-bold text-[#1c63d5] hover:text-red-500 hover:underline">
    {" "}
    {text}{" "}
  </Link>
);
...
const ModalDetail: React.FC<IDragProps> = ({ modal, issue, mousePosition, zIndex, onClick }) => {
  console.log(issue);
  ...
  const buttons = [
    {
      label: "details",
      onClick: () => navigate(`/issues/${issue.id}`), // Navigate to the detail view
    },
    {
      label: "edits",
      onClick: () => {
  ...
  return (
    <Draggable onMouseDown={() => onClick()}>
      <div className="bg-white fixed border rounded" ref={modalRef} style={modalStylePosition}>
        ...
        <div className="detail overflow-auto h-[600px]">
          <div className="bg-yellow-50 border p-2 m-2">
            <div className="flex items-center">
              ...
              <div className="ps-2">
                ...
                <p className={textSize}>
                  Added by
                  {links.map((link, index) => renderLink(link.to, link.text, index))}
                </p>
              </div>
            </div>
            ...
            {sections.map((section) => renderSection(section.title, section.content, section.isLink, section.link, section.onClick))}
            {showRelatedIssueForm && (
              <form className="flex items-center text-13 gap-1 mt-2">
                ...
                <input type="text" placeholder="Input text" className="w-full border border-solid border-[#d7d7d7] p-1 rounded" />
