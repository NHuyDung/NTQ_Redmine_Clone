...
const Roadmap: React.FC<RoadmapProps> = ({ identifier }) => {
  ...
  useEffect(() => {
    const fetchProjectVersions = async () => {
      try {
        setLoading(true);
        const { data: projectVersionsData, projectId } = await getProjectVersions(identifier);
  ...
  const fetchProjectVersionsIssues = async (projectId: number) => {
    try {
      setLoading(true);
      const projectVersionsData: ProjectVersionsIssuesResponse = await getProjectVersionsIssues(projectId);
